  ...
  return (
    <>
      ...
      <div style={{ minHeight: '100vh', background: '#000', color: 'white', display: 'flex', flexDirection: 'column' }}>
        <div style={{ position: 'fixed', top: 0, left: '50%', transform: 'translateX(-50%)', width: 900, height: 600, pointerEvents: 'none', zIndex: 0, background: 'radial-gradient(ellipse 900px 500px at 50% -5%, rgba(139,0,0,0.07) 0%, transparent 70%)' }} />

        <header style={{ position: 'sticky', top: 0, zIndex: 100, borderBottom: '1px solid rgba(255,255,255,0.05)', background: 'rgba(0,0,0,0.92)', backdropFilter: 'blur(24px)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 40px', height: 56, flexShrink: 0 }}>
          <a
            href={platformUrl ? `${platformUrl}/study` : '/study'}
            style={{ display: 'flex', alignItems: 'center', gap: 7, fontFamily: F.inter, fontSize: 12, letterSpacing: '0.02em', color: 'rgba(255,255,255,0.28)', textDecoration: 'none', transition: 'color 0.2s' }}
        ...
        <footer style={{ borderTop: '1px solid #3D0000', padding: '20px 40px', display: 'flex', justifyContent: 'center', flexShrink: 0, position: 'relative', zIndex: 1 }}>
...
function StatusIcon({ type }) {
  const configs = {
    check: { stroke: '#8B0000', path: <path d="M20 6L9 17l-5-5" />, bg: 'rgba(139,0,0,0.08)', border: 'rgba(139,0,0,0.3)', shadow: '0 0 28px rgba(139,0,0,0.18)' },
    clock: { stroke: 'rgba(255,255,255,0.3)', path: <><circle cx="12" cy="12" r="9" /><path d="M12 7v5l3 3" /></>, bg: 'rgba(255,255,255,0.03)', border: 'rgba(255,255,255,0.08)', shadow: 'none' },
    lock: { stroke: 'rgba(255,255,255,0.18)', path: <><rect x="3" y="11" width="18" height="11" rx="2" /><path d="M7 11V7a5 5 0 0110 0v4" /></>, bg: 'rgba(255,255,255,0.02)', border: 'rgba(255,255,255,0.06)', shadow: 'none' },
  };
  const c = configs[type] || configs.check;
  return (
    <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ duration: 0.5, delay: 0.1, ease }} style={{ width: 52, height: 52, borderRadius: '50%', border: `1px solid ${c.border}`, background: c.bg, boxShadow: c.shadow, display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 28px' }}>
      <svg width="19" height="19" viewBox="0 0 24 24" fill="none" stroke={c.stroke} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">{c.path}</svg>
    </motion.div>
  );
}
...
function StateLabel({ children }) {
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5, delay: 0.04, ease }} style={{ marginBottom: 20, textAlign: 'center' }}>
      <span style={{ fontFamily: F.space, fontSize: 10, fontWeight: 600, color: 'rgba(255,255,255,0.2)', letterSpacing: '0.16em', textTransform: 'uppercase' }}>{children}</span>
    </motion.div>
  );
}
...
function DescriptionCard({ lines }) {
  ...
  return (
    <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 16, marginBottom: 28, overflow: 'hidden' }}>
      <div style={{ padding: '22px 28px', borderLeft: '2px solid #3D0000', borderBottom: lines.length > 1 ? '1px solid rgba(255,255,255,0.05)' : 'none' }}>
        <p style={{ fontFamily: F.inter, fontWeight: 400, fontSize: 15, color: 'rgba(255,255,255,0.75)', lineHeight: 1.75, margin: 0 }}>{lines[0]}</p>
      </div>
      {lines.length > 1 && (
        <div style={{ padding: '22px 28px', display: 'flex', flexDirection: 'column', gap: 14 }}>
          {lines.slice(1).map((line, i) => (
            <p key={i} style={{ fontFamily: F.inter, fontWeight: 300, fontSize: 14, color: 'rgba(255,255,255,0.45)', lineHeight: 1.8, margin: 0 }}>{line}</p>
          ))}
        </div>
      )}
    </div>
...
function CTAButton({ href, label }) {
  return (
    <a href={href} style={{ display: 'inline-flex', alignItems: 'center', gap: 9, fontFamily: F.inter, fontWeight: 600, fontSize: 14, color: 'white', background: '#8B0000', padding: '14px 38px', borderRadius: 10, textDecoration: 'none', letterSpacing: '0.02em', boxShadow: '0 0 36px rgba(139,0,0,0.2), inset 0 1px 0 rgba(255,255,255,0.06)', transition: 'background 0.2s, box-shadow 0.2s, transform 0.15s' }} onMouseEnter={e => { e.currentTarget.style.background = '#9e0000'; e.currentTarget.style.boxShadow = '0 0 56px rgba(139,0,0,0.38), inset 0 1px 0 rgba(255,255,255,0.06)'; e.currentTarget.style.transform = 'translateY(-1px)'; }} onMouseLeave={e => { e.currentTarget.style.background = '#8B0000'; e.currentTarget.style.boxShadow = '0 0 36px rgba(139,0,0,0.2), inset 0 1px 0 rgba(255,255,255,0.06)'; e.currentTarget.style.transform = 'none'; }}>
      {label}
...
export default function Convite({ participant, error, blocked, platformUrl }) {
  ...
  return wrap(
    <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease }} style={{ maxWidth: 600, width: '100%' }}>
      ...
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.18, ease }}>
