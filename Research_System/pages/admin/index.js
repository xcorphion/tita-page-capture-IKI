...
export default function AdminPanel() {
  ...
  const fetchParticipants = async (pwd) => {
    try {
      const res = await fetch('/study/api/admin/participants', {
    ...
    <div style={{ minHeight: '100vh', background: '#080808', color: 'white', fontFamily: F.inter }}>
      ...
      <div style={{ padding: '32px 40px', overflowX: 'auto' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 900 }}>
          <thead>
            <tr>
              {['Código', 'Nome', 'Indicador', 'Status', 'Sessão 1', 'Sessão 2', 'Sessão 3', 'Ações'].map(h => (
          ...
          <tbody>
            {participants.map(p => (
              ...
              >
                ...
                <td style={tdStyle}>
                  <span style={{
                    display: 'inline-block', fontFamily: F.inter, fontSize: 10, fontWeight: 600,
                    letterSpacing: '0.08em', textTransform: 'uppercase',
                    padding: '3px 8px', borderRadius: 4,
                    color: p.status === 'ATIVO' ? 'rgba(120,200,120,0.9)' : p.status === 'BLOQUEADO' ? 'rgba(255,255,255,0.3)' : 'rgba(200,80,80,0.9)',
                    background: p.status === 'ATIVO' ? 'rgba(120,200,120,0.08)' : p.status === 'BLOQUEADO' ? 'rgba(255,255,255,0.04)' : 'rgba(200,80,80,0.08)',
                    border: `1px solid ${p.status === 'ATIVO' ? 'rgba(120,200,120,0.2)' : p.status === 'BLOQUEADO' ? 'rgba(255,255,255,0.1)' : 'rgba(200,80,80,0.2)'}`,
                  }}>{p.status}</span>
                </td>
                {[
                  ...
                  <td key={i} style={tdStyle}>
                    <span style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 13 }}>
                      {s.status || '—'}
                      {(s.devMismatch || s.ipMismatch) && (
                        <span
                          title={[s.devMismatch && 'Device diferente', s.ipMismatch && 'IP diferente'].filter(Boolean).join(' · ')}
                          style={{ fontSize: 11, color: 'rgba(220,140,60,0.9)', cursor: 'default' }}
                        >
                          ⚠
                        </span>
                      )}
                    </span>
                    <span style={{ fontFamily: F.inter, fontSize: 11, color: 'rgba(255,255,255,0.3)', marginTop: 2, display: 'block' }}>
                      Eng: {s.eng === true ? 'Sim' : s.eng === false ? 'Não' : '—'}
                    </span>
                  </td>
                ))}
            ...
            {participants.length === 0 && (
              <tr>
                <td colSpan={8} style={{ ...tdStyle, textAlign: 'center', color: 'rgba(255,255,255,0.2)', padding: '48px 16px' }}>
