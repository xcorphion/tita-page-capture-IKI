...
export default function Dashboard() {
  ...
  const fetchData = async (authPwd) => {
    ...
    try {
      const url = session_id
        ? `/study/api/analysis/report?session_id=${session_id}`
        : '/study/api/analysis/report';
    ...
    return (
      <div style={{ minHeight: '100vh', background: '#080808', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
        ...
        <div style={{ width: '100%', maxWidth: 380, background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 16, padding: '40px 36px' }}>
          <h2 style={{ fontFamily: F.space, fontWeight: 700, fontSize: 22, color: 'white', letterSpacing: '-0.025em', marginBottom: 28 }}>
          ...
          <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            <button type="submit" style={{
              ...
              cursor: 'pointer', boxShadow: '0 0 20px rgba(139,0,0,0.22)',
  ...
  if (loading || !data) {
    return (
      <div style={{ minHeight: '100vh', background: '#080808', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <span style={{ fontFamily: F.inter, fontSize: 13, color: 'rgba(255,255,255,0.3)', letterSpacing: '0.08em' }}>
          Carregando análise estatística...
        </span>
      </div>
    );
  }
  ...
  return (
    <div style={{ padding: '0 0 60px', background: '#080808', minHeight: '100vh', color: '#fff' }}>
      <Head>
        <title>Pipeline de Análise — Xcorphion</title>
      </Head>
      ...
      <header style={{
        ...
        <span style={{ fontFamily: F.space, fontSize: 13, fontWeight: 600, color: 'rgba(255,255,255,0.7)', letterSpacing: '-0.01em' }}>
          Pipeline de Análise{' '}
          <span style={{ color: '#8B0000' }}>IKIs × EMAs</span>
        </span>
        <div style={{ display: 'flex', gap: 10 }}>
          <button
            onClick={() => window.location.href = '/study/api/analysis/export-csv'}
            style={{
              fontFamily: F.inter, fontWeight: 500, fontSize: 11,
          ...
          <button
            ...
            style={{
              ...
              padding: '6px 14px', cursor: 'pointer',
              letterSpacing: '0.06em', textTransform: 'uppercase',
              boxShadow: '0 0 16px rgba(139,0,0,0.2)',
      ...
      <div style={{ padding: '32px 40px 0' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 28 }}>
          ...
          <MetricCard
            title="Pearson r (Médio)"
            value={stats.current_pearson.toFixed(3)}
            color={Math.abs(stats.current_pearson) >= 0.4 ? 'rgba(120,200,120,0.9)' : 'rgba(220,140,60,0.9)'}
            gate="Gate 2: r ≥ 0.4"
            met={Math.abs(stats.current_pearson) >= 0.4}
          />
          <MetricCard
            title="Cohen's d (Terciles)"
            value={stats.current_cohens_d.toFixed(3)}
            color={stats.current_cohens_d >= 0.5 ? 'rgba(120,200,120,0.9)' : 'rgba(220,140,60,0.9)'}
            gate="Gate 1: d ≥ 0.5"
            met={stats.current_cohens_d >= 0.5}
          />
          <MetricCard
            title="Status do Dataset"
            value={stats.current_cohens_d >= 0.5 ? 'VALIDADO' : 'COLETANDO'}
            color={stats.current_cohens_d >= 0.5 ? 'rgba(120,200,120,0.9)' : 'rgba(220,140,60,0.9)'}
          />
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          ...
          <PlotCard title="Trajetória no Circumplexo de Russell">
            <Plot
              data={[{
                ...
                line: { shape: 'spline', color: '#8B0000' },
                marker: { size: 10, color: plots.russell.map(p => p.char), colorscale: 'Reds' },
              }]}
              layout={plotLayout({ xTitle: 'Valência', yTitle: 'Arousal', xRange: [0, 100], yRange: [0, 100] })}
          ...
          <PlotCard title="IKI Timeline (Escala Log) + Marcadores EMA">
            <Plot
              data={[
                {
                  ...
                  mode: 'markers',
                  marker: { size: 2, color: 'rgba(255,255,255,0.15)' },
                  name: 'IKI (log1p)',
                },
              ...
              layout={plotLayout({ xTitle: 'Tempo Relativo (ms)', yTitle: 'log1p(IKI)', showLegend: false })}
              style={{ width: '100%' }} config={{ responsive: true }}
            />
          </PlotCard>
          ...
          <PlotCard title="Distribuição Lognormal dos IKIs">
            <Plot
              data={[{
                ...
                marker: { color: 'rgba(139,0,0,0.45)' },
                histnorm: 'probability density',
              }]}
              layout={plotLayout({ xTitle: 'log1p(IKI)', yTitle: 'Densidade' })}
              style={{ width: '100%' }}
            />
          </PlotCard>
            ...
            <Plot
              data={[
                ...
                },
                {
                  ...
                  y: [0.4, 0.4],
                  mode: 'lines',
                  line: { color: 'rgba(220,140,60,0.7)', dash: 'dash' },
                  name: 'Gate 2',
                },
              ]}
              layout={plotLayout({ xTitle: 'N Participantes', yTitle: 'Pearson r', yRange: [-1, 1] })}
              style={{ width: '100%' }} config={{ responsive: true }}
            />
          </PlotCard>
              ...
              data={[
                {
                  ...
                  mode: 'lines',
                  line: { color: 'rgba(120,200,120,0.8)', width: 4 },
                },
                {
                  x: [0, stats.total_sessions],
                  y: [0.5, 0.5],
                  mode: 'lines',
                  line: { color: 'rgba(200,80,80,0.7)', dash: 'dash', width: 2 },
                  name: 'Gate 1',
                },
              ]}
              layout={plotLayout({ xTitle: 'N Participantes', yTitle: "Cohen's d", yRange: [0, 2] })}
              style={{ width: '100%' }} config={{ responsive: true }}
            />
          </PlotCard>
...
function PlotCard({ title, children, span }) {
  return (
    <div style={{
      ...
      borderRadius: 12, padding: '20px 20px 4px',
...
function MetricCard({ title, value, color = 'white', gate = '', met }) {
  return (
    <div style={{
      background: 'rgba(255,255,255,0.02)',
      border: `1px solid ${met === true ? 'rgba(120,200,120,0.18)' : met === false ? 'rgba(200,80,80,0.12)' : 'rgba(255,255,255,0.06)'}`,
      borderRadius: 12, padding: '20px',
    }}>
      <div style={{ fontFamily: F.inter, fontSize: 10, fontWeight: 600, color: 'rgba(255,255,255,0.3)', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 10 }}>
        {title}
      </div>
      <div style={{ fontFamily: F.space, fontSize: 28, fontWeight: 700, color, letterSpacing: '-0.025em', lineHeight: 1 }}>
        {value}
      </div>
      {gate && (
        <div style={{ fontFamily: F.inter, fontSize: 10, color: 'rgba(255,255,255,0.2)', marginTop: 8 }}>
          {gate}
        </div>
      )}
    </div>
